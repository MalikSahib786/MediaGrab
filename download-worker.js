/**
 * MediaGrab - Video/Image Downloader Worker
 * Handles downloads from YouTube, Instagram, Twitter, TikTok, Facebook, Pinterest
 */
...
async function handleYouTube(url) {
  try {
    ...
    // All available thumbnail qualities
    const thumbnails = {
      maxresdefault: {
...
async function handleInstagram(url) {
  try {
    ...
    // Method 1: Try Instagram's API endpoint
...
async function handleTwitter(url) {
  try {
    ...
    // Use Twitter's syndication API (public endpoint)
    ...
    try {
      ...
      if (response.ok) {
        ...
        return {
          ...
          embedHtml: data.html,
          // Note: TikTok doesn't provide direct download via oEmbed
...
async function handlePinterest(url) {
  try {
    ...
    // Extract image/video URLs from JSON-LD or meta tags
...
async function handleRequest(request, env, ctx) {
  ...
  if (url.pathname === '/health' || url.pathname === '/') {
    return new Response(JSON.stringify({ 
      status: 'ok', 
      service: 'MediaGrab Download API',
      version: '1.0.0',
      endpoints: ['/api/download', '/api/platforms', '/health']
    }), {
      headers: {
